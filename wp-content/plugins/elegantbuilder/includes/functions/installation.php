...
<div class="indent"><?php _e( 'You can change the images used in each of the advertisements, as well as which URL each ad points to, through the custom option pages found in wp-admin. Once logged in to the wordpress admin panel, click &quot;Design&quot; and then &quot;Chameleon Theme Options&quot; to reveal the various theme options. You can also use the 125x125 advertisement widget by adding the ET: Advertisement widget to your sidebar, and filling in the required fields.', $themename ); ?></div>
  ...
  <span class="boldtext"><?php esc_html_e( 'How do I set up the featured slider on the homepage?', $themename ); ?></span>
  <div class="indent">
  <p><?php _e( 'The featured slider can be set up using two different methods. You can either populate the tabs using Pages, or you can popular it using posts from a designated category. In the Appearances > Chameleon Theme Options page in wp-admin, under the General Settings > Featured Slider tab, you will see an option that says "Use Pages." If you select this option then pages will be used, if you don\'t then posts will be used. If you want to use Pages then you simply select "Use Pages," and then below the option select which pages you would like to display in the slider. If you don\'t use pages, then simply select the "Featured Category" from the dropdown menu and posts from that category will be added to the slider. You can also choose between three different slider variations (Nivo, Cycle and Roundabout). YOu can switch between each slider type via the "Slider Type" setting located in the General Settings > Featured Slider tab of ePanel.', $themename ); ?></p></div>
  
  <span class="boldtext"><?php esc_html_e( 'Setting Up The Media Bar On The Homepage', $themename ); ?></span> 
  <div class="indent"> 
  <p><?php _e( 'Each item in the media bar on the homepage comes from a post. All of these posts are pulled from a single category that is defined within the ePanel theme options. The first thing you need to do is choose which category you would like to pull your posts from. To do this, navigate to the Appearances > Chameleon Theme Options page and click on the General Settings > Homepage tab. Look for the "Exclude categories from homepage media section" section and select, with a green checkmark, which categories you would like to display in the media bar.', $themename ); ?>
</p> 
<p><?php _e( 'Next you will need to add "media" to each of the posts within those categories. To add an image to the media bar, you simply need to add the Thumbnail custom field to each of the posts as outlined in the "Adding Thumbnails Images To Posts" tutorial above.', $themename ); ?></p>
<p><?php _e( 'You can also add video to each of the posts in the media bar. To do this you will need to add a new custom field to the post with a Name of "et_videolink" and a Value of the URL to your video. You can only embed videos from video hosts such as youtube.com or vimeo.com. Once you have chosen a video, simply add the URL to the video in the "Value" field of the custom field. For example, if I wanted to add the following video to the post: http://vimeo.com/21294655 then all I would do is type "http://vimeo.com/21294655" into the Value field of the "et_videolink" custom field.', $themename ); ?></p>
</div> 
  
  <span class="boldtext"><?php esc_html_e( 'How do I setup the three blurbs on the homepage, below the slider?', $themename ); ?></span>
  <div class="indent">
  <p><?php _e( 'These blurbs are created using pages. Create three pages that you would like to use for these three blurbs, then go to the Appearances > Chameleon Theme Options page and click the General Settings > Homepage tab. Under the Service Page 1/2/3 dropdown menus, choose the three pages you would like to use. Wi tin the pages you can use a "more" tag to truncate the post preview.', $themename ); ?>
</p>
<p><?php _e( 'Next you will need to assign an Icon image to each blurb. This works just like Thumbnail images, except you use the custom field name "Icon" instead. So create a custom field for each page with the name of "Icon" and a value of the icon image you would like to use. Icons must not exceed 36x36 pixels to insure optimum display.', $themename ); ?></p>
</div>
