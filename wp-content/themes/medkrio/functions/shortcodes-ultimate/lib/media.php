<?php
	...
	function su_get_media( $media_url, $width, $height, $preview = false ) {
		...
		if ( in_array( $image_ext, $images ) ) {
			$return = '<img src="' .   get_template_directory_uri()  . '/functions/shortcodes-ultimate/lib/timthumb.php?src=' . $media_url . '&amp;w=' . $width . '&amp;h=' . $height . '&amp;zc=1&amp;q=100" alt="" width="' . $width . '" height="' . $height . '" />';
		}
		...
		if ( in_array( $video_ext, $videos ) ) {
			...
			$return = '
					...
					<script>
					flowplayer("player' . $player_id . '", {src:"' .   get_template_directory_uri()  . '/functions/shortcodes-ultimate/lib/flowplayer-3.2.7.swf", wmode: \'Opaque\'}, {
					screen:	{
		bottom: 0	
	},
	plugins: {
		controls: {
			url: \'' .  get_template_directory_uri() . '/functions/shortcodes-ultimate/lib/flowplayer.controls-3.2.5.swf\',
...
<script>
	$f("audio' . $player_id . '", {src:"' . get_template_directory_uri() . '/functions/shortcodes-ultimate/lib/flowplayer-3.2.7.swf", wmode: \'Opaque\'}, {
	// fullscreen button not needed here
	plugins: {
		audio: {
			url: \'' . get_template_directory_uri() . '/functions/shortcodes-ultimate/lib/flowplayer.audio-3.2.2.swf\'
		},
		controls: {
			fullscreen: false,
			height: 30,
